...
the headline number, is the payoff of building a real eval. (The full per-case audit is in
`evals/error_audit.md`.)
...
the prompt. I did exactly that, adding an explicit "a firm winning a specific contract is
...
- **Model-asserted ground truth:** the labels were attached by the generator, not verified
  by a human, so some "misclassifications" may be bad labels. The error audit found that most
  category misses sit on this kind of label-boundary overlap, not clean model mistakes.
...
Over three passes, category accuracy's run-to-run std is **0.19 points** (`evals/stability.txt`),
so the ~2x-std threshold for a meaningful difference is about 0.4 points. The reverted
experiment's 2.3-point drop is roughly **12x** that noise floor, confirming it was a real
