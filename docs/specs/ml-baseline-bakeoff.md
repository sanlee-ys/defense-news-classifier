...
2. **There is no region label in the training data.** The v2.1.0 scaled eval covered category and domain only; the scaled region eval is unscheduled. So the baseline covers **two axes, not three**, and the writeup must say so rather than quietly omitting region.
