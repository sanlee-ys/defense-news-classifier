...
**Author:** San Lee
**Last updated:** 2026-08-02 (§12 appended at verdict)
...
## 12. Results and verdict *(appended 2026-08-02, after the run)*
...
[`evals/region_clause_ab.txt`](../../evals/region_clause_ab.txt) (scale) and
[`evals/region_clause_gold_eval.txt`](../../evals/region_clause_gold_eval.txt) (gold, frozen
under a `region_clause_` name because the shipped v3.0.0 gold record was restored — see
ADR-023's *Where the candidate's gold numbers live*).

### 12.1 Arm 1 — scale, effective n=295

Answer key: the frozen `claude-opus-4-8` judge column, digest `346b905682342ed8`, **not
re-run** (§4.1). Duplicates excluded from the pairing exactly as §2.1 required (s023, s025,
s079, s080, s240).

| Axis | Baseline | Candidate | Lift | cand/base/tie | McNemar p | Registered as |
|---|---|---|---|---|---|---|
| **region** | 88.5% | **92.2%** | **+3.7%** | 19 / 8 / 268 | **0.0522** | target |
| category | 91.5% | 92.2% | +0.7% | 6 / 4 / 285 | 0.7539 | guardrail |
| operational_domain | 89.5% | 93.2% | +3.7% | 15 / 4 / 276 | 0.0192 | guardrail |

| Named-cluster accounting | |
|---|---|
| Named pulls in the baseline | 17 |
| **F** — fixed | **12** (71%) — s126, s157, s129, s134, s067, s141, s288, s256, s131, s263, s259, s107 |
...
Harness health clean on all three axes: 295 groups / 295 pairs / 295 eligible.

**Against §2.3's pre-run estimates.** F was predicted ≤13 and landed at 12 — but four of the
four rows §2.3 said the narrowing had given up (s126, s067, s271, s283) split two and two:
**s126 and s067 were fixed anyway**, s271 and s283 were not. B was predicted at ≈9 plausible
and landed at 7, of which §2.3 had named three in advance (s101, s116, s251). The estimates
were close, and §6.1's "the realistic operating point is a marginal result" was correct.

### 12.2 Arm 2 — gold, n=54, human labels

| Measure | Baseline (shipped v3.0.0) | Candidate |
|---|---|---|
| **Region accuracy** | 87.0% (47/54) | **94.4%** (51/54) |
| Region macro-F1 | 0.927 | 0.964 |
| `global` recall / precision | 0.632 / 1.000 | **1.000** / 0.905 |
| Category accuracy | 92.6% | 94.4% |
| Operational domain accuracy | 92.6% | 94.4% |
| Judge-vs-human **region** agreement | 100.0% | 96.3% (2: g024, g054) |
| Judge-vs-human category / domain | 92.6% / 98.1% | 96.3% / 96.3% |

Row level: **all seven** named misses fixed (g013, g017, g019, g026, g047, g048, g054);
**three** correct `americas` rows broken — g024 → `global`, g037 → `global`, g030 →
`middle-east`. Net +4.

§5 predicted **g024** as the single plausible break under the narrowed clause and was right;
g030 and g037 were not predicted. §5 also expected only the five institution-only rows to be
reachable — the clause additionally fixed g013 (`7th Fleet` in a career history) and g054 (a
`NEWPORT NEWS, Va.` dateline), both form-2 rows it was narrowed *away* from.

**Gold-arm exposure, resolved.** §5 called `judge_region_agreement` a realistic risk rather
than a tail risk. It moved 1.000 → 0.963 — 2 disagreements against a budget of 3 — so the
0.93 floor held. No floor in `evals/thresholds.toml` was breached on any axis.

### 12.3 The rule, applied

| Rule (§6) | Result | Pass? |
|---|---|---|
| 1. Scale region `F − B > 0` **and p < 0.05** | +11 net, **p = 0.0522** | ❌ **fails by 0.0022** |
| 2. No significant guardrail harm | category p=0.75; domain **improved** +3.7% (p=0.019) — kill condition never fired | ✅ |
| 3. Gold region ≥ 87.0%, no floor breached | 94.4%; every floor clear | ✅ |
| 4. Harness health clean, 295 eligible | 295/295 × 3 | ✅ |

Three of four pass and the fourth misses by 0.0022, which is precisely the case §6 named in
advance: *"Call it MARGINAL, and revert, when region improves but p ≥ 0.05."* San ruled on
2026-08-02 to honor the rule. A pre-registration that binds only when convenient is not one.

**The domain improvement is recorded, not banked.** Domain was registered as a guardrail, so
its kill condition could fire but its improvement could not count toward shipping — reading an
unregistered gain as support is the outcome-switching the pre-registration exists to prevent.
It is unexplained and it is the most interesting thing to register properly in any follow-up.
ADR-023 carries that observation in full.

### 12.4 What §9's "negative or marginal" branch actually did

All four steps, as written, plus one the spec left as an owner call:

1. Clause reverted — `src/classify.py` back to `prompt_sha256 a59689e8…`, which also returns
   `gen_metrics_artifact.py` and `src/eval_gate.py` to green with no waiver.
2. The pre-run gold artifacts restored from git (§9's recommendation, taken): the published
   record must not be re-baselined under a prompt that does not ship. **The owner call §9
   left open** — where the paid run's raw gold data then goes — was resolved by freezing it
   under `evals/region_clause_gold_candidate.csv` and `evals/region_clause_gold_eval.txt`
   rather than only quoting it, so the data itself survives the revert.
3. ADR-023 records the negative result — the sixth measure-first data point.
4. No version bump, no marker cascade, no threshold change.
