...
- **Tool use (`tool_choice: "tool"`)** forces the model to call a named tool with a defined JSON schema. The API validates the response against that schema before returning it — out-of-enum or malformed responses are rejected at the API layer, not caught after the fact in application code. This is the key reliability mechanism for both `classify.py` and `generate.py`.
